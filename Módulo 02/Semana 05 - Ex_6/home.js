...
let listaDeDados = JSON.parse(sessionStorage.getItem("lista"));

function gerarCards(dados) {
  const listagemCards = document.getElementById("listagem-cards");
  listagemCards.innerHTML = dados.map((dado, index) => `
...
document.querySelector(".busca > input").addEventListener("keyup", (e) => {
  ...
  const dadosFiltrados = listaDeDados.filter((dado) => dado.titulo.toLowerCase().includes(filtro) | dado.subtitulo.toLowerCase().includes(filtro)
  );
  gerarCards(filtro ? dadosFiltrados : listaDeDados);
});


document.querySelector(".busca > button").addEventListener("click", () => {
  const input = prompt("Digite o que deseja buscar");
  const filtro = input.toLowerCase();
  const dadosFiltrados = listaDeDados.filter((dado) => dado.titulo.toLowerCase().includes(filtro) || dado.subtitulo.toLowerCase().includes(filtro)
  );
  document.querySelector(".busca > input").value = input;
  gerarCards(input ? dadosFiltrados : listaDeDados);
});
